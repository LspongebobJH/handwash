...
video_path = 'IMG_2375.mp4'
fps = 30
wind_leng = 25
image_size=(720, 1280)
interpolation='bilinear'
num_channels = 3
saved_skelton_video = "test_video_with_keypoints_1020.mp4"
saved_wash_step_video = "test_video_with_wash_step_1020.mp4"
    ...
    with mp_hands.Hands(
        ...
        if results.multi_hand_landmarks:
            ...
            for m, hand_landmarks in enumerate(results.multi_hand_landmarks):
                # get the handless of this hand
                handless = results.multi_handedness[m].classification[0]
                landmarks = hand_landmarks.landmark

                # write handless and hand_landmarks into txt file
                with open('keypoint.txt', 'a') as f:
                    f.write(str(handless))
...
def split_and_prep_frame(video_path):
    ...
    videoWrite = cv2.VideoWriter(saved_skelton_video, fourcc, 25, (1280,720))
    
    while is_success:
        is_success, image = vid_cap.read()
        
        if is_success:
            # write frame number into txt file
            with open('keypoint.txt', 'a') as f:
                f.write('Frame:'+str(frame_num))
                f.write('\n')

            # get hand key points
            image, key_points = get_hand_key_point(image)
            
            # write filltered keypoint into txt file
            with open('filltered_keypoint.txt', 'a') as f:
                f.write('Frame:'+str(frame_num))
                f.write('\n')
    ...
    #split keypoint_tensor into small pieces
    keypoint_input = []
